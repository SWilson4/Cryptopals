...
func BreakRepeatingKeyXOR(file *os.File) (key, plaintext string, err error) {
	base64Ciphertext, err := ioutil.ReadAll(file)
	if err != nil {
		return
	}

	rawCiphertext := make([]byte, base64.StdEncoding.DecodedLen(len(base64Ciphertext)))
	bytesRead, err := base64.StdEncoding.Decode(rawCiphertext, base64Ciphertext)
	if err != nil {
		return
	}

	keySize, err := findKeySize(rawCiphertext[:bytesRead])
	if err != nil {
		return
	}
	...
	rawPlaintext := repeatingKeyXOR(rawCiphertext[:bytesRead], rawKey)
